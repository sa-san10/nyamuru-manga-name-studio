...
test('プロンプト§BのパターンA〜Yを選択肢として持つ', () => {
  assert.equal(PANEL_TEMPLATES.map((template) => template.id).join(''), 'ABCDEFGHIJKLMNOPQRSTUVWXY');
  assert.deepEqual(PANEL_TEMPLATES.map((template) => template.shapes.length), [3, 6, 1, 4, 5, 4, 4, 6, 4, 5, 5, 3, 1, 4, 3, 4, 4, 4, 4, 5, 3, 3, 4, 4, 4]);
