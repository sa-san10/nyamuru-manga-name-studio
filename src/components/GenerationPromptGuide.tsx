...
export default function GenerationPromptGuide({ lang, onChangeLang, onNotify }: Props) {
  ...
  return <DownloadableMarkdownGuide
    ...
    eyebrow="GENERATION PROMPT · NDM v10.3"
    title={en ? 'Manga Name Generation Prompt' : '漫画ネーム生成プロンプト'}
    description={en
      ? 'The full prompt (written in Japanese) that structures a story manuscript into NDM v10.3 and outputs pure name data in OMNY format. Artwork rules for image generation are managed separately as the OMAY file (see the generation workflow tab).'
      : 'ストーリー原稿をNDM v10.3へ構造化し、純粋なネームデータをOMNY形式で出力するための完全版プロンプトです。画像生成向けの作画ルールは、OMAYファイルとして分離管理します（生成ワークフロータブ参照）。'}
