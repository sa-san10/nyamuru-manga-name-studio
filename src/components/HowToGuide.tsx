...
const STEPS: {
  ...
  },
  {
    ...
    tasks: [
      ...
      'NDM検査でルール違反がないか確かめて、「OMNY保存」からOMNYファイルを保存し直す（画像生成指示分離版と全部入り版を選べます）',
    ],
  ...
  },
  {
    number: '3',
    title: '画像生成AIで漫画にする',
    tool: '「生成ワークフロー」タブのワークフロー ＋ 「作品情報」タブの画像生成指示（OMAY） ＋ キャラ・背景のリファレンス画像 ＋ 画像生成できるAIエージェント（GPT Workなど）',
    tasks: [
      '「生成ワークフロー」タブのワークフローと、「作品情報」タブの画像生成指示（OMAY）を、エージェントが参照できる場所に配置してプロジェクト指示を設定する（初回のみ。以降の作品はOMNYを渡すだけ）',
      ...
      <div class="howto-flow">
        ...
        <aside class="howto-tip">
          ...
          <div>
            ...
            <p>このアプリに入っているサンプル漫画をヘッダーの「OMNY保存」でOMNYファイルにして、「にゃむるたん」タブの立ち絵画像と一緒にエージェントへ渡せば、STEP 3だけをすぐ体験できます（スタジオを経由せず一枚で完結させたいときは全部入り版が便利です）。</p>
          </div>
        </aside>
        <aside class="howto-tip">
